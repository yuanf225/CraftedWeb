    ...
    if ( isset($_POST['item_entry']) )
    {
        ...
        else
        {
            ...
            if ( $row['sendType'] == "ra" )
            {
                ...
                $Server->sendRA("send items ". $Character->getCharname($realm[0]) ." \"Your requested item\" \"Thanks for supporting us!\" ". $entry ." ", $row['rank_user'], $row['rank_pass'], $row['host'], $row['ra_port']);
                $statement->close();
            }
            elseif ( $row['sendType'] == "soap" )
            {
                ...
                $Server->sendSoap("send items ". $Character->getCharname($realm[0]) ." \"Your requested item\" \"Thanks for supporting us!\" ". $entry ." ", $row['rank_user'], $row['rank_pass'], $row['host'], $row['soap_port']);
                $statement->close();
            }
        }
    }
